    ...
    {else}
        ...
        <table class="list_table" style="width:100%" cellpadding="1" cellspacing="1">
            <tr style="height: 20px;">
                {if $results|@count > 1}
                    <th width="40">{$LANG.word_order}</th>{/if}
                <th>{$LANG.phrase_field_label}</th>
                <th>{$LANG.phrase_field_type}</th>
                <th>{$L.phrase_field_identifier}</th>
            ...
            {foreach from=$results item=field name=row}
                ...
                <tr>
                    {if $results|@count > 1}
                        <td align="center"><input type="text" name="field_{$id}_order" size="3"
                                                  value="{$field.field_order}"/></td>{/if}
                    <td class="pad_left_small">{$field.field_label}</td>
                    <td class="pad_left_small">{$field.field_type|ucwords}</td>
                    <td class="pad_left_small grey">{$field.field_identifier|default:'&#8212;'}</td>
                    <td align="center">
